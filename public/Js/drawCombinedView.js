...
export function drawCombinedView(context, photoNumber) {

  context.ctxCombined.clearRect(0, 0, context.canvasCombined.width*1.5, context.canvasCombined.height);

  console.log('drawCombinedView:', context.scopeOn);

  // Calculate dimensions and positions based on canvas width
  const canvasWidth = context.canvasCombined.width*1.2;
  const canvasHeight = context.canvasCombined.height;

  // Calculate dimensions for Video A (camElement.top)
  const videoAWidth = canvasWidth * 0.35; // Video A width is 25% of canvas width
  const videoAHeight = videoAWidth * (3 / 4); // Maintain 4:3 aspect ratio

  // Calculate dimensions for Video B (camElement.scope) to cover 75% of the canvas width
  const videoBWidth = canvasWidth * 0.65; // Video B width is 75% of canvas width
  ...
  // Draw Video A (camElement.top)
  context.ctxCombined.drawImage(
    camElement.top, // Image source
  ...
  const currentCamElement = camElement.scope 
  if (!currentCamElement) {
    currentCamElement = camElement.top 
    return null;
  }

  // Apply cropping and draw Video B (currentCamElement)
  const sourceWidth = SCOPE_DRAW_WIDTH 
  const sourceHeight = SCOPE_DRAW_HEIGHT
  ...
  const activeLabel = '--';
  const textBoxBContent = `Saved Photo #${photoNumber}\nRecorded Id: ${context.recordId} (${activeLabel})\nTime: ${currentTime}\n`;
  wrapText(context.ctxCombined, textBoxBContent, 5, canvasHeight - 50, canvasWidth - 20, fontSize);

  // Return the combined canvas image as a data URL
  const imageDataUrl = context.canvasCombined.toDataURL('image/jpeg');
  return imageDataUrl;
}
